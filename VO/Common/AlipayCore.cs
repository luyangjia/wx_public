...
namespace EMP2016.API.VO.Common
{
    ...
    public class Core
    {
        public  ConfigAliPay config;
        public Core(ConfigAliPay config)
        {
            this.config = config;
        }
        ...
        /// <returns>拼接完成以后的字符串</returns>
        public  string CreateLinkStringUrlencode(Dictionary<string, string> dicArray, Encoding code)
        {
            StringBuilder prestr = new StringBuilder();
            foreach (KeyValuePair<string, string> temp in dicArray)
            {
        ...
        public  string GetAbstractToMD5(Stream sFile)
        {
        ...
        /// <returns>MD5摘要结果</returns>
        public  string GetAbstractToMD5(byte[] dataFile)
        {
            MD5 md5 = new MD5CryptoServiceProvider();
